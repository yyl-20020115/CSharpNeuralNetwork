...
namespace Accord.MachineLearning.Rules
{
        ...
        IUnsupervisedLearning<AssociationRuleMatcher<T>, T[], T[][]>
    {
        ...
        public AssociationRuleMatcher<T> Learn(SortedSet<T>[] x, double[] weights = null)
        {
            ...
            foreach (var s in x.SelectMany(t => t))
            ...
            for (int k = 1; L.Count != 0; k++)
            {
                var C = fold(L, k);
                counts.Clear();
                foreach (SortedSet<T> candidate in from transaction in x from candidate in C where candidate.IsSubsetOf(transaction) select candidate)
                {
                    int count;
                ...
                L.Clear();
                foreach (var pair in counts.Where(pair => pair.Value >= supportMin))
                {
                    L.Add(pair.Key);
