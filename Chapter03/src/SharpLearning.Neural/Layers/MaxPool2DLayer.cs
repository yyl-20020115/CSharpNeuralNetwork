...
namespace SharpLearning.Neural.Layers
{
    ...
    public sealed class MaxPool2DLayer : ILayer
    {
        ...
        public void Initialize(int inputWidth, int inputHeight, int inputDepth, int batchSize, Initialization initializtion, Random random)
        {
            ...
            // computed
            Depth = InputDepth;
            Width = ConvUtils.GetFilterGridLength(InputWidth, m_poolWidth, m_stride, m_padWidth, BorderMode);
            Height = ConvUtils.GetFilterGridLength(InputHeight, m_poolHeight, m_stride, m_padHeight, BorderMode);

            // store switches for x,y coordinates for where the max comes from, for each output neuron
            Switchx = Enumerable.Range(0, batchSize).Select(v => new int[Width * Height * Depth]).ToArray();
            Switchy = Enumerable.Range(0, batchSize).Select(v => new int[Width * Height * Depth]).ToArray();
        ...
        public void CopyLayerForPredictionModel(List<ILayer> layers)
        {
            ...
            copy.Switchx = Enumerable.Range(0, batchSize).Select(v => new int[Width * Height * Depth]).ToArray();
            copy.Switchy = Enumerable.Range(0, batchSize).Select(v => new int[Width * Height * Depth]).ToArray();
