...
namespace SharpLearning.Ensemble.Learners
{
    ...
    public sealed class RegressionStackingEnsembleLearner : ILearner<double>, IIndexedLearner<double>
    {
        ...
        public RegressionStackingEnsembleLearner(IIndexedLearner<double>[] learners, Func<F64Matrix, double[], IPredictorModel<double>> metaLearner,
            ICrossValidation<double> crossValidation, bool includeOriginalFeaturesForMetaLearner = true)
        {
            m_learners = learners ?? throw new ArgumentException("learners");
            m_crossValidation = crossValidation ?? throw new ArgumentException("crossValidation");
            m_metaLearner = metaLearner ?? throw new ArgumentException("metaLearner");
